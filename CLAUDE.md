...
## Stack Tecnológico
- **Orquestación**: CrewAI, LangGraph, LangChain
- **API Gateway**: FastAPI 0.115+, Uvicorn
- **Messaging**: RabbitMQ 3.12+ (eventos y workflows asíncronos)
- **Base de datos**: PostgreSQL 16, Alembic (migrations)
- **Cache/Session**: Redis 7
- **Storage**: MinIO (S3 compatible) para grabaciones y documentos
- **LLMs**: OpenAI, Claude, Ollama (configurable)
- **Infraestructura**: Docker Compose (local), Kubernetes (prod)
- **Observabilidad**: Prometheus, Grafana, Sentry

## Estructura Principal
- **`agents/`**: Agentes especializados (LinkedIn, Email, Finance, HR, Marketing, Cold Calling) vinculados como git submodules
- **`api_gateway/`**: FastAPI servidor central que expone `/api/v1` y coordina agents
- **`astramech-orchestrator/`**: Supervisor CrewAI y workflows de LangGraph
- **`company-efficiency-optimizer/`**: Aplicación frontend/UI para monitoreo
- **`external_repos/`**: Repositorios externos integrados como submódulos (incluye `betibot` y `supervincent`)
- **`config/`**: Configuraciones y variables de entorno
- **`docker-compose.yml`**: Stack completo (PostgreSQL, Redis, RabbitMQ, MinIO)

## Comandos Útiles
```bash
# Instalar dependencias de agentes y servicios
pip install -r api_gateway/requirements.txt
pip install -r astramech-orchestrator/requirements.txt
...
python astramech-orchestrator/main.py

# Verificar arquitectura
cat ARCHITECTURE.md

# Revisar status de agentes
cat AGENTS_STATUS.md
...
- **[docs/ml-notes/](docs/ml-notes/)** — Notas de referencia de ciencia de datos (cheatsheets, resúmenes)

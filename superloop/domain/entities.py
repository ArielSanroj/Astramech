...
class Afirmacion:
    """R3 — un hecho/inferencia/supuesto/pregunta, siempre etiquetado."""
    texto: str
...
@dataclass
class Producto:
    """Unidad de negocio (§2). `tipo`: dominio_kpi | agente."""
    producto_id: str
    nombre: str
    tipo: str = "dominio_kpi"
    owner: str | None = None
...
class DecisionRecomendada:
    """§6.3 / §11 — decisión aprobable con respaldo (R6)."""
    decision_id: str
    producto_id: str
    fecha: str
    fase_origen: str
    ...
    def to_row(self) -> dict[str, Any]:
        """Aplana al esquema §11 (JSON donde corresponde)."""
        return {
            ...
            "opciones_consideradas": json.dumps(self.opciones_consideradas),
            "razonamiento": self.razonamiento,
            "datos_usados": json.dumps(self.datos_usados),
            "afirmaciones": json.dumps([a.to_dict() for a in self.afirmaciones]),
...
@dataclass
class Aprendizaje:
    """§6.7 — alimenta el próximo DECIDE (R8)."""
    producto_id: str
    hipotesis: str
    resultado: str
    aprendizaje: str
...
class RegistroCanonicoEntry:
    """§10 — estado actual de un producto."""
    producto_id: str

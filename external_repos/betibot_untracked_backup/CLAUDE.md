# BetiBot - Sistema Inteligente de Procesamiento de Facturas

## Descripción
BetiBot es un sistema automatizado de procesamiento de facturas que integra OCR avanzado, detección inteligente de tipos de factura (compra/venta), extracción de datos precisos y sincronización automática con la plataforma contable Alegra. El sistema procesa PDFs e imágenes, valida cumplimiento fiscal, genera reportes contables completos (Libro Mayor, Balance de Prueba, Diario General) y mantiene auditoría detallada de todas las operaciones.

## Stack Tecnológico
- **Lenguaje**: Python 3.11+
- **Procesamiento de Documentos**: pdfplumber, Tesseract OCR, OpenCV
- **APIs**: Alegra (integración contable), DIAN (validación fiscal), Nanobot (conversacional)
- **Tareas Asincrónicas**: Celery, Redis, Kombu
- **Monitoreo**: Watchdog (file system), Prometheus, Flower
- **Validación**: Pydantic v2
- **Testing**: pytest, pytest-cov, pytest-asyncio
- **Calidad**: Black, isort, mypy, flake8, bandit

## Estructura Principal
- **src/core/processors/**: Procesadores principales de facturas (enhanced_invoice_processor.py)
- **src/core/parsers/**: Parsers para PDF (pdf_parser.py) e imágenes (image_parser.py)
- **src/core/ocr/**: Módulos OCR avanzado (ollama_enhanced_ocr.py, pdf_enhanced_ocr.py)
- **src/core/validators/**: Validadores de impuestos y entrada de datos
- **src/integrations/**: Integraciones con Alegra, DIAN y Nanobot
- **src/tasks/**: Tareas asincrónicas con Celery
- **src/utils/**: Utilidades de logging, configuración y monitoreo
- **config/**: Archivos de configuración por entorno
- **scripts/**: Scripts de utilidad para análisis y operación
- **tests/**: Suite de tests unitarios e integración
- **reports/**: Reportes contables generados
...
python -m src.core.processors.enhanced_invoice_processor process /path/to/invoice.pdf
python invoice_watcher.py /path/to/facturas/
...
flower -A src.tasks.celery_tasks --port=5555

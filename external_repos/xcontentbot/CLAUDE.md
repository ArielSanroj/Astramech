...
## Descripción
Bot automatizado para descubrir y interactuar con posts en X (Twitter) sobre burnout, bienestar de empleados y productividad, utilizando comentarios generados por IA. Integra múltiples proveedores de LLM (OpenAI, Anthropic), gestión de estado, circuit breakers y monitoreo comprensivo.
...
- **Monitoring**: Prometheus, Grafana API, InfluxDB
- **Database**: SQLite/PostgreSQL con SQLAlchemy
- **Scheduling**: APScheduler
- **Development**: pytest, black, flake8, mypy, pre-commit

## Estructura Principal
- **bot_runner.py**: Orquestador principal y scheduling del bot
- **x_client.py**: Cliente principal de X (integración con x_mcp.py)
- **x_mcp.py**: Servidor MCP para interacción con plataforma X
- **llm_orchestrator.py**: Gestión multi-proveedor de LLM
- **circuit_breaker.py**: Patrones de resiliencia
- **throttling.py**: Rate limiting e throttling
- **cache_manager.py**: Sistema de caché de alto rendimiento
- **database.py**: Capa de persistencia con SQLAlchemy
- **observability.py**: Monitoreo y logging
- **tests/**: Suite completa de tests (unit, integration, playwright)
- **scripts/**: Scripts de utilidad

## Comandos Útiles
```bash
# Setup
make install                 # Instalar dependencias + Playwright + pre-commit hooks
playwright install chromium # Descargar navegador Chromium

# Desarrollo
make test                    # Ejecutar tests con cobertura (>80% required)
make lint                    # Validar sintaxis (flake8, mypy)
make format                  # Formatear código (black, isort)
make clean                   # Limpiar cache y archivos temporales

# Ejecución
make run                     # Ejecutar: x_mcp.py (servidor) + x_client.py (bot)
python x_mcp.py             # Iniciar servidor MCP manualmente
python x_client.py          # Ejecutar bot manualmente

# Testing específico
pytest -m unit              # Solo tests unitarios
pytest -m integration       # Solo tests de integración
pytest -m playwright        # Solo tests que requieren Playwright
pytest --cov               # Con reporte de cobertura HTML

# Revisión de código
bash scripts/ai-review.sh   # Validación automática de calidad
bash scripts/cto-check.sh   # Verificación de CTO Rules
```
